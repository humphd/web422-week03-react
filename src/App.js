...
function App() {
  ...
  const [pages, setPages] = useState({ current: 1 });
  ...
  const { current } = pages;
  useEffect(() => {
    loadUsers([current]).then((result) => {
      setPages({ current: result.page, total: result.total_pages });
  ...
  const handlePageChange = (page) => setPages({ ...pages, current: page });
